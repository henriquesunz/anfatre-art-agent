...
function masterFontSize(text, box, baseSize, minSize, lineHeightFactor) {
  ...
  // 0,78 é conservador de propósito: o Canva não executa o autofit do PowerPoint.
  const horizontal = usableWidth * 72 / (longest * 0.78);
  ...
  return Math.max(minSize, Math.min(baseSize, Math.floor(horizontal), Math.floor(vertical)));
}
...
function addPhotoMasterPanel(slide, tone, intro, highlight, highlightFirst = false) {
  ...
  const introText = balanceMasterLines(intro, Math.max(1, introLines), 35);
  const highlightText = balanceMasterLines(highlight, Math.max(1, highlightLines), 17);
...
function buildPhotoSignature(slide, plan, image) {
  ...
  const title = balanceMasterLines([copy.intro, copy.highlight].filter(Boolean).join(" "), 3, 22);
