...
export function balanceLines(value, maxLines, preferredChars) {
...
export function fontForLines(text, baseSize, preferredChars, minSize) {
...
export function masterLineCount(value, preferredChars, maxLines = 3) {
  const text = normalizedText(value);
  if (!text) return 0;
...
export function balanceMasterLines(value, maxLines, preferredChars) {
  const text = normalizedText(value);
  if (!text) return "";
  const words = text.split(" ");
...
export function masterFontSize(text, box, baseSize, minSize, lineHeightFactor, charEm = 0.65) {
  const lines = String(text || "").split("\n").filter(Boolean);
  const longest = Math.max(...lines.map((line) => line.length), 1);
  const usableWidth = Math.max(0.1, box.w - 0.2);
  const usableHeight = Math.max(0.1, box.h - 0.1);
  // charEm = largura média de um caractere em "em". 0,65 é o padrão calibrado para o
  // fluxo Canva; pesos mais largos (ExtraBold caixa alta) devem passar um valor maior.
  const horizontal = usableWidth * 72 / (longest * charEm);
