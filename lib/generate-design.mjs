...
function addPhotoPanel(slide, tone, intro, highlight, highlightFirst = false) {
  ...
  if (!normalizedText(first) || !normalizedText(second)) {
    const single = normalizedText(first || second);
    const singleIsHighlight = first ? firstIsHighlight : !firstIsHighlight;
    const singleWrapped = balanceLines(single, 4, singleIsHighlight ? 23 : 28);
    addText(slide, singleWrapped, {
      x: 1.45,
      y: 1.73,
      w: 7.1,
      h: 3.55,
      fontSize: fontForLines(singleWrapped, singleIsHighlight ? 42 : 35, singleIsHighlight ? 23 : 28, 25),
      bold: singleIsHighlight,
      color: singleIsHighlight ? COLORS.yellow : COLORS.white,
      align: "center",
      valign: "mid",
      breakLine: true,
      fit: "shrink",
