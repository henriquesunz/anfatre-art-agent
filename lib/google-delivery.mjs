...
function shapeFontSize(element) {
  for (const item of element.shape?.text?.textElements || []) {
    const size = item.textRun?.style?.fontSize?.magnitude;
    if (size) return size;
  }
  return null;
}
...
async function deliverPhotoFromMaster(accessToken, plan, image, title, uploader) {
  ...
  for (const element of target.pageElements || []) {
    ...
    // replaceAllText preserva o estilo do texto original (fonte, peso, cor).
    requests.push({
      replaceAllText: {
        containsText: { text: existing, matchCase: false },
        replaceText: replacement,
        pageObjectIds: [target.objectId],
      },
    });
    const baseSize = shapeFontSize(element);
    if (replacement && baseSize) {
      const { width, height } = elementSizePt(element);
      const fitted = fitFontSize(replacement, width, height, baseSize);
      if (fitted < baseSize) {
        requests.push({
          updateTextStyle: {
            objectId: element.objectId,
            style: { fontSize: { magnitude: fitted, unit: "PT" } },
            textRange: { type: "ALL" },
            fields: "fontSize",
          },
        });
