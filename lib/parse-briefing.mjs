...
function unlabeledCopy(block) {
  ...
  if (lines.length <= 1) return { title: oneLine(content), subtitle: "" };
  return { title: oneLine(lines[0]), subtitle: compact(lines.slice(1).join("\n")) };
}

function splitTitleAndSubtitle(title, explicitSubtitle = "") {
  const exactTitle = oneLine(title);
  const exactSubtitle = oneLine(explicitSubtitle);
  if (exactSubtitle || !exactTitle) return { title: exactTitle, subtitle: exactSubtitle };
  ...
  return { title: exactTitle, subtitle: "" };
}
...
function itemFromBlock(date, rawBlock, index) {
  ...
  const coverCopy = carouselSlides.length
    ? splitTitleAndSubtitle(slides[0]?.title || "", copy.subtitle)
    : splitTitleAndSubtitle(copy.title, copy.subtitle);
